...
def run_cmake_build(build_dir, target="all"):
    idf_env = os.environ.copy()
    populate_idf_env_vars(idf_env)
    ...
    if rc != 0:
        sys.stderr.write(
            "Error: Couldn't execute native CMake build target `%s`.\n" % target
        )
        env.Exit(1)
...
# `pio test` still compiles sources via SCons. On a fresh IDF6 test build, some
# projects do not materialize `config/sdkconfig.h` during configure alone, so
# SCons can start compiling managed components before the generated config
# headers exist. If this happens, run one native build pass to force the config
# artifacts into place before SCons continues.
if "__test" in COMMAND_LINE_TARGETS and not os.path.isfile(
